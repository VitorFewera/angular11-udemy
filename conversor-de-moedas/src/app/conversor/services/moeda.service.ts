...
export class MoedaService {
  ...
  private moedas: Moeda[]; // lista de moedas

  constructor() {}

  private moedasObj = [ //http://fixer.io
	{ "sigla": "AUD", "descricao": "Dólar australiano" }, // base de dados local
  ...
  listarTodas(): Moeda[] { //converter a base de dados para uma listagem de moedas
  	if (this.moedas) { //se a moeda foi selecionada, retorna a moeda - cash
  		return this.moedas;
  	} 
  	
  	this.moedas = []; // transforma moedas em um array

  	for (let moedaObj of this.moedasObj) {
  		let moeda: Moeda = new Moeda();
  		Object.assign(moeda, moedaObj); // Pega um objeto de origem "moeda" e passa para um objeto 'destino'
  		this.moedas.push(moeda); // adiciona a moeda na listagem
  	}

  	return this.moedas; // retornar o moedas que virou o array
  }
